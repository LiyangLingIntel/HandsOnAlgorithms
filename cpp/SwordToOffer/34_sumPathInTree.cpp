...
// 思路还是递归的方法, 知道某条路径能满足加起来之和为 expectNumber,
// 就把当前路径放到结果集合中. 需要注意的是在构建路径的时候,
// 完成某条路之后需要在递归弹栈时同时把路径中的节点也弹出去.
class Solution {
  public:
    void FindPath(TreeNode *node, int expectNumber, vector<vector<int>> &res,
                  vector<int> &path) {
